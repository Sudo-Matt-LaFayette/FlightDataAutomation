...
//    }
    ...
    public void doSomething() throws InterruptedException, AWTException, IOException {
        ...
        for (int x = 0; x < destination.length; x++) {//{ 1; x++) {//
            ...
            // Begin date loops
            // do larger for loop to only do 3 months? May, June July
            for (int z = 0; z < 3; z++) {
                ...
                for (int i = 10; i < (daysInCurrentMonth - 6); i++) { // this will run the EXACT needed amt of times per month
                    ...
                    System.out.println("I am running the 'i' for loop " + (getNumTimesToRunForMonth() - 21 - 6) + " times");
                    ...
                    //System.out.println("end day + 1 = " + (endDay) + " & getNumTimesToRunForMonth(calendar) = " + getNumTimesToRunForMonth());
                    ...
                    if (monthName.getText().equals(fullMonthString)) {
                        ...
                        if (monthName.getText().equals("August 2021")) {
                            // We're done here!
                        ...
                        System.out.println("\nThe y loop will run " + (getNumTimesToRunForMonth() - 21 - 6) + " times");
                        for (int y = 0; y < (getNumTimesToRunForMonth() - 6); y++) {

                            // If we've found the first day... click on it along with the last day
                            System.out.println("The value of y is " + y);
                            System.out.println((days.get(y).getAttribute("data-day")) + " is the text and " + (startDay) + " is the target #");
                            if (days.get(y).getAttribute("data-day").equals(String.valueOf((startDay)))) {
                                System.out.println(days.get(y).getAttribute("data-day") + " is the beginning day");
                                days.get(y).click();
                                Thread.sleep(2000);
                                System.out.println(days.get(y + 7).getAttribute("data-day") + " is the end day");
                                days.get(y + 7).click();
                                break;
                            }
                        }
